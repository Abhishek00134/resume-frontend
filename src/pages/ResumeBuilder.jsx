...
const ResumeBuilder = () => {
  ...
  const changeResumeVisibility = async ()=>{
    try {
      ...
      const {data} = await api.put("/api/resumes/update", {headers: {Authorization: token}})
  ...
  const saveResume = async () => {
    try {
      ...
      console.log("IMAGE TYPE:", resumeData.personal_info.image);
        ...
        <div className='relative lg:col-span-5 rounded-lg overflow-hidden'>
            <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
                ...
                <hr className='absolute top-0 left-0 h-1 bg-linear-to-r from-green-500 to-green-600 border-none transition-all duration-2000' style={{width:`${activeSectionIndex*100/(sections.length -1)}%`}} />
                  ...
                  <button onClick={()=>{toast.promise(saveResume, {loading: 'Saving...'})}} className='bg-linear-to-br from-green-100 to-green-400 transition-all rounded-md px-6 py-2 mt-6 text-sm '>
                    Save Changes
                  </button>
            </div>
        </div>
        ...
        <div className='lg:col-span-7 max-lg:mt-6'>
          <div className='relative w-full'>
              ...
              <div className='absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2'>
                      {resumeData.public && (
                        <button onClick={handleShare}className='flex items-center p-2 px-4 gap-2 text-xs bg-linear-to-br from-blue-100 to-blue-200 text-blue-600 rounded-lg ring-blue-300 hover:ring'>
                          <Share2Icon className='size-4' /> Share
                        </button>
                      )}
                      <button onClick={changeResumeVisibility} className='flex items-center p-2 px-4 gap-2 text-xs bg-linear-to-br from-blue-100 to-purple-200 text-purple-600 ring-purple-300 rounded-lg hover:ring transition-colors'>
